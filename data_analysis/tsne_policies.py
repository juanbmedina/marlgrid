...
def plot_tsne_panel(
    ...
    plot_tsne(emb, df, "role",         ax=axes[0], title="By role")
    plot_tsne(emb, df, "agent_id",     ax=axes[1], title="By agent")
    plot_tsne(emb, df, "action_price", ax=axes[2], title="By action price (action[1])")

    if perplexity is not None:
        fig.suptitle(f"t-SNE of policy observations for {case} (perplexity = {perplexity:.0f})",
                     fontsize=14)
    fig.tight_layout()
...
class PolicyTSNE:
    ...
    def panel(self, perplexity: float = 30, figsize=None, save_path=None, case=None):
        ...
        return plot_tsne_panel(emb, self.df, perplexity=perplexity,
                               save_path=save_path, case=case,**kwargs)
